...
--   				__Note:__ The node (and any relevant children) should be @CanvasItem.visible@ when returned, otherwise, the viewport that instantiates it will not be able to calculate its minimum size reliably.
...
-- | Forces drag and bypasses @method _get_drag_data@ and @method set_drag_preview@ by passing @data@ and @preview@. Drag will start even if the mouse is neither over nor pressed on this control.
...
--   			__Note:__ If the Control node is a child of a @Container@ node, the scale will be reset to @Vector2(1, 1)@ when the scene is instantiated. To set the Control's scale when it's instantiated, wait for one frame using @yield(get_tree(), "idle_frame")@ then set its @rect_scale@ property.
...
-- | Invalidates the size cache in this node and in parent nodes up to top_level. Intended to be used with @method get_minimum_size@ when the return value is changed. Setting @rect_min_size@ directly calls this method automatically.
...
-- | Sets @offset_left@ and @offset_top@ at the same time. Equivalent of changing @rect_position@.
